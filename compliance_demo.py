...
class ArchivistStoryRunner:
    ...
    def __init__(self, url, token_file, config_file):
        ...
        with open(token_file, mode="r") as tokenfile:
            ...
            self.client = Archivist(url, auth=token, verify=True)
    ...
    def run_operation(self, operation, args, to_print=None, wait_time=0):
        ...
        if operation == Operation.LOAD_ASSET.name:
            self.load_asset(
                args["asset_id"],
                args["identity"]
            )
    ...
    def load_asset(self, asset_id, identity):
        """Fetches an asset and adds it to the
           asset list.

            Args:
                asset_id (string): the unique local asset identity.
                identity (string): the RKVST identity of the asset to load
        """

        asset = self.client.assets.read(identity)
        print("Asset Loaded!")
        self.assets[asset_id] = asset
